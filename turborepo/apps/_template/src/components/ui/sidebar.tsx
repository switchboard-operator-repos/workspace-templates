...
function SidebarProvider({
  ...
  const setOpen = React.useCallback(
    (nextState: boolean | ((current: boolean) => boolean)) => {
      ...
      if (setOpenProp) {
        setOpenProp(resolvedState);
      } else {
        _setOpen(resolvedState);
      }

      // Persist the sidebar state without using document.cookie directly.
      try {
        ...
        if (cookieStoreObj?.set) {
          void cookieStoreObj.set({
            name: SIDEBAR_COOKIE_NAME,
            value: String(resolvedState),
            expires: Date.now() + SIDEBAR_COOKIE_MAX_AGE * 1000,
            path: "/",
          });
        } else {
          // Fallback to localStorage to avoid direct cookie assignment
          localStorage.setItem(SIDEBAR_COOKIE_NAME, String(resolvedState));
        }
      } catch {
        // no-op
      }
